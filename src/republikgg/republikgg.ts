...
export class RepublikGGAPI {
    ...
    _updateProfile = async (newValue: string, type: string): Promise<any> => {
        ...
        try {
            const response = await axios.put(
                `${BASE_API_URL}/production/profile/${this.userId}/`, defaultPayload, {
                headers: {
                    "Authorization": `Bearer ${this.authToken}`,
                    ...this._getHeaders()
                }
    ...
    Self = {
        ...
        follow: async (userId: string): Promise<void> => {
            ...
            try {
                const response = await axios.post(`${BASE_API_URL}/production/profile/${userId}/followers`, {}, {
                    headers: {
                        "Authorization": `Bearer ${this.authToken}`,
                        ...this._getHeaders()
                    }
        ...
        like: async (activityId: string): Promise<void> => {
            let data: any | undefined = undefined;
            try {
                const response = await axios.post(`${BASE_API_URL}/production/activity-likes`, { activityId }, {
                    headers: {
                        "Authorization": `Bearer ${this.authToken}`,
                        ...this._getHeaders()
                    }
